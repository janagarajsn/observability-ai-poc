...
LOGS_PATH = os.getenv("LOGS_PATH", "./input-logs/*.json")
INGESTION_TRACKER_FILE = os.getenv("INGESTION_TRACKER_FILE", "./ingestracker/ingested_files.json")
...
def ingest_logs(qdrant_client: QdrantClient):
    ...
    if not log_files:
        logger.warning(f"No log files found at path: {LOGS_PATH}")
        return
